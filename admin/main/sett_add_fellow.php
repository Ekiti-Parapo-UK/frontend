...
if ($_GET['typ'] == "new") {
    ...
    if ($title == "" || $name == "" || $yea == "" || $name == "" || $phone == "" || $email == "" || $bio == "") {
        display_error("Please, specify the the field marked *");
    } else if ($ezzzy->getrecords("dir_scholars", "email", $email) != 0) {
        display_error("Oops!! It seems you have added this scholar before");
    } else {
        //all is well
        if (is_uploaded_file($_FILES['pic']['tmp_name'])) {
            ...
            $fields = "title,,yea,,name,,sex,,mstatus,,religion,,nationality,,dob,,phone,,email,,facebook,,twitter,,linkedin,,bio,,picture";
            $vals = "'$title',,'$yea',,'$name',,'$sex',,'$mstatus',,'$religion',,'$nation',,'$dob',,'$phone',,'$email',,'$facebook',,'$twitter',,'$linkedin',,'$bio',,'$filest'";
            $result = $ezzzy->addlist($fields, $vals, "dir_fellows");
            if (!$result) {
            ...
            //we shall now insert the refords
            $fields = "title,,yea,,name,,sex,,mstatus,,religion,,nationality,,dob,,phone,,email,,facebook,,twitter,,linkedin,,bio";
            $vals = "'$title',,'$yea',,'$name',,'$sex',,'$mstatus',,'$religion',,'$nation',,'$dob',,'$phone',,'$email',,'$facebook',,'$twitter',,'$linkedin',,'$bio'";
            $result = $ezzzy->addlist($fields, $vals, "dir_fellows");
            if (!$result) {
                $_SESSION["msg"] = '<font color=red><b>Invalid Operation! </b> ' . ' No action taken. Try again<br><br></font>';
            }
    ...
    if ($ntitle == "" || $ndesc == "") {
        ...
        $fields = "title,,yea,,name,,sex,,mstatus,,religion,,nationality,,dob,,phone,,email,,facebook,,twitter,,linkedin";
        $vals = "$title,,$yea,,$name,,$sex,,$mstatus,,$religion,,$nation,,$dob,,$phone,,$email,,$facebook,,$twitter,,$linkedin";
        $result = $ezzzy->updateval($fields, $vals, "dir_fellows", "id", $rec);
        if (!$result) {
            $_SESSION["msg"] = '<font color=red><b>Invalid Operation! </b> ' . ' No action taken. Try again<br><br></font>';
        }
    }
