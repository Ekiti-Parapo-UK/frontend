...
$edate = trim(sql_safe($_POST["edate"]));
$etime = trim(sql_safe($_POST["etime"]));
$evenue = trim(sql_safe($_POST["even"]));
$ndesc = trim(($_POST["ndesc"]));
$saddr = $_POST["saddr"]; //this is the server address
if ($_GET['typ'] == "new") {

    if ($ntitle == "" || $edate == "" || $etime == "" || $evenue == "" || $ndesc == "") {
        display_error("Please, specify the Event Title and hence provide a description for the Event");
    } else if ($ezzzy->getrecords("news", "title", $ntitle) != 0) {
        display_error("Oops!! It seems you have added this Event itme before");
    } else {
        //all is well
        if(isset($_POST['soption']) && $_POST['soption'] == 1){
            $soption = "YES";
        }
        if (is_uploaded_file($_FILES['pic']['tmp_name'])) {
            ...
            $filest = "uploads/" . $newpic;
            /*             * ****************************************************************************************** */
            //we shall now insert the refords
            $fields = "title,,descp,,ntype,,dates,,times,,venue,,adt,,picture,,soption";
            $vals = "'$ntitle',,'$ndesc',,'EVENT',,'$edate',,'$etime',,'$evenue',,'$now',,'$filest',,'$soption'";
            ...
            //we shall now insert the refords
            $fields = "title,,descp,,ntype,,dates,,times,,venue,,adt,,soption";
            $vals = "'$ntitle',,'$ndesc',,'EVENT',,'$edate',,'$etime',,'$evenue',,'$now',,'$soption'";
            $result = $ezzzy->addlist($fields, $vals, "news");
            if (!$result) {
                $_SESSION["msg"] = '<font color=red><b>Invalid Operation! </b> ' . ' No action taken. Try again<br><br></font>';
            }
    ...
    if ($ntitle == "" || $ndesc == "") {
        ...
        $fields = "title,,descp,,dates,,times,,venue";
         $vals = "$ntitle,,$ndesc,,$edate,,$etime,,$evenue";
