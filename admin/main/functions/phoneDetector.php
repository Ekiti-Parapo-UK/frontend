...
class NigerianPhone {
    ...
    public function isValid() {
        ...
        if (substr($this->line, 0, 3) == '234') {
            ...
            elseif (strlen(( $this->line ) == 11)) {
                ...
                if (in_array(substr($stripCcc, 0, 2), $this->areaCodes) && strlen($stripCcc) == 7) {
        ...
        # doesn't contain country calling code
            ...
            elseif (substr($this->line, 0, 1) == 0 && in_array(str_replace(substr($this->line, 0, 1), "", substr($this->line, 0, 3)), $this->areaCodes) && strlen($this->line) == 8) {
                $this->isValidLandLine = true;
    ...
    public function isMultilinks() {
        return ( in_array(substr($this->formatted(), 0, 5), self::$allPrefixes['multilinks']) || in_array(substr($this->formatted(), 0, 4), self::$allPrefixes['multilinks']) );
    }

    public function isVisafone() {
        return ( in_array(substr($this->formatted(), 0, 5), self::$allPrefixes['visafone']) || in_array(substr($this->formatted(), 0, 4), self::$allPrefixes['visafone']) );
    }
    ...
    public function isStarcomms() {
        return ( in_array(substr($this->formatted(), 0, 5), self::$allPrefixes['starcomms']) || in_array(substr($this->formatted(), 0, 4), self::$allPrefixes['starcomms']) );
    }
