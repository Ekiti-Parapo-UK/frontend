    ...
    */
    ...
    function showpic($where,$whereval,$typ){
      ...
      //let us get the image of this user
        ...
        * @return boolean
        */
        public function getrecords($table,$where="",$whereval="",$whereb="",$wherevalb="",$wherec="",$wherevalc="",$whered="",$wherevald="") {
            // declare
            $binds = array();
            //checks
            ...
            if(!empty($whered) && !empty($wherevald)){
                $binds[":wherevald"] = $wherevald;
                $mqry = "SELECT COUNT(*) AS mycnt FROM " . $table ." WHERE ". $where ."=:whereval AND ". $whereb ."=:wherevalb AND ". $wherec ."=:wherevalc AND ".$whered."=:wherevald";
            }
            // Any binds
            if(!empty($binds)){
                $mqry .= ";";
        ...
        * @return boolean
        */
        public function getsum($table,$sumfield="",$where="",$whereval="",$whereb="",$wherevalb="",$wherec="",$wherevalc="",$whered="",$wherevald="") {
            // declare
            $binds = array();
            //checks
            if(!empty($where) && !empty($whereval)){
            ...
            if(!empty($whered) && !empty($wherevald)){
                $binds[":wherevald"] = $wherevald;
                $mqry = "SELECT SUM(".$sumfield.") AS mycnt FROM " . $table ." WHERE ". $where ."=:whereval AND ". $whereb ."=:wherevalb AND ". $wherec ."=:wherevalc AND ". $whered ."=:wherevald";
            }
            // Any binds
            if(!empty($binds)){
                $mqry .= ";";
        ...
        * @return boolean
        */
        public function getsumbtw($btw1,$btw2,$field,$table,$sumfield=0,$where="",$whereval="",$whereb="",$wherevalb="",$wherec="",$wherevalc="",$whered="",$wherevald="") {
            // declare
            //$binds = array();
            $binds = array(
                ":btw1" => "$btw1",
            ...
            if(!empty($where) && !empty($whereval)){
                $binds[":whereval"] = $whereval;
                //$mqry = "SELECT COUNT(*) AS mycnt FROM " . $table ." WHERE ". $where ."=:whereval";
                $mqry = "SELECT SUM(".$sumfield.") AS mycnt FROM " . $table ." WHERE ". $where ."=:whereval AND (".$mwhere.")";
            }
            if(!empty($whereb) && !empty($wherevalb)){
                $binds[":wherevalb"] = $wherevalb;
                //$mqry = "SELECT COUNT(*) AS mycnt FROM " . $table ." WHERE ". $where ."=:whereval AND ". $whereb ."=:wherevalb";
                $mqry = "SELECT SUM(".$sumfield.") AS mycnt FROM " . $table ." WHERE ". $where ."=:whereval AND ".$whereb."=:wherevalb AND (".$mwhere.")";
            }
            if(!empty($wherec) && !empty($wherevalc)){
                $binds[":wherevalc"] = $wherevalc;
                $mqry = "SELECT SUM(".$sumfield.") AS mycnt FROM " . $table ." WHERE ". $where ."=:whereval AND ". $whereb ."=:wherevalb AND ". $wherec ."=:wherevalc AND (".$mwhere.")";
            }
            if(!empty($whered) && !empty($wherevald)){
                $binds[":wherevald"] = $wherevald;
                $mqry = "SELECT SUM(".$sumfield.") AS mycnt FROM " . $table ." WHERE ". $where ."=:whereval AND ". $whereb ."=:wherevalb AND ". $wherec ."=:wherevalc AND " .$whered. "=:wherevald AND (".$mwhere.")";
            }
            // Any binds
            if(!empty($binds)){
                $mqry .= ";";
        ...
        */
        ...
        public function canTransact($mid,$table,$times){
            $gdet = $this->getOne("SELECT id,dates from $table WHERE cid='".$mid."' order by id desc");
            //$refd = strtotime($gdet['dates']);
            //$refd = floor((time() - strtotime($gdet['dates']))/86400); //this is the number of seconds since the transaction has occured
            $refd = time() - strtotime($gdet['dates']); //this is the number of seconds since the transaction has occured
            if($refd >=$times){ //thie times is in seconds
                return true;
            }
            else{
                return false;
        ...
        public function airtimeLimit($mid,$what,$limitamt,$reference){
            $now = date("Y-m-d");
            $gdet = $this->getOne("SELECT sum(amount) as thamt from billsairtimedata WHERE cid='$mid' and reference='$reference' and descp like '%$what%' and dates like '%$now%'");
            
            $refd = $gdet['thamt'];
            if($refd > $limitamt){ //thie times is in seconds
                return true;
            }
            else{
                return false;
        ...
        /**
         * This function will return the wallet balance of any customer
         * 
         * @param type $mid
         * @param type $phid
         * @return integer
         */
        public function getWalletBalance($mid){
            $trades = $this->getsum("trades","namount","cid",$mid,"status","SUCCESS"); //credit
            $benefits = $this->getsum("benefits","amount","cid",$mid); //credit
            $payinfo = $this->getsum("payinfo","amount","regno",$mid,"status","SUCCESS"); //credit from funding wallet
            $withdraws = $this->getsum("withdraws","amount","cid",$mid,"status","SUCCESS") + $this->getsum("withdraws","amount","cid",$mid,"status","PENDING") + $this->getsum("withdraws","amount","cid",$mid,"status","NEW"); // debit
            $spin = $this->getsum("giveaways","amount","cid",$mid); // debit
            $airtimedatabill = $this->getsum("billsairtimedata","amount","cid",$mid,"status","SUCCESS") + $this->getsum("billsairtimedata","amount","cid",$mid,"status","PENDING","hstatus","YES"); // debit
            $bal = ($trades + $benefits + $payinfo) - ($withdraws + $spin + $airtimedatabill);
            return $bal;
        }//end of the function that returns the wallet balace
        
        public function getWalletBalancePerTrans($mid,$btw1,$btw2,$field){
            //getsumbtw($btw1,$btw2,$field,$table,$sumfield="",$where="",$whereval="",$whereb="",$wherevalb="",$wherec="",$wherevalc="",$whered="",$wherevald="")
            $trades = (float)$this->getsumbtw($btw1,$btw2,$field,"trades","namount","cid",$mid,"status","SUCCESS"); //credit
            $benefits = (float)$this->getsumbtw($btw1,$btw2,$field,"benefits","amount","cid",$mid); //credit
            $payinfo = (float)$this->getsumbtw($btw1,$btw2,$field,"payinfo","amount","regno",$mid,"status","SUCCESS"); //credit from funding wallet
            $withdraws = (float)$this->getsumbtw($btw1,$btw2,$field,"withdraws","amount","cid",$mid,"status","SUCCESS") + (float)$this->getsumbtw($btw1,$btw2,$field,"withdraws","amount","cid",$mid,"status","PENDING") + (float)$this->getsumbtw($btw1,$btw2,$field,"withdraws","amount","cid",$mid,"status","NEW"); // debit
            $spin = (float)$this->getsumbtw($btw1,$btw2,$field,"giveaways","amount","cid",$mid); // debit
            $airtimedatabill = (float)$this->getsumbtw($btw1,$btw2,$field,"billsairtimedata","amount","cid",$mid,"status","SUCCESS") + (float)$this->getsumbtw($btw1,$btw2,$field,"billsairtimedata","amount","cid",$mid,"status","PENDING","hstatus","YES"); // debit
            $bal = ($trades + $benefits + $payinfo) - ($withdraws + $spin + $airtimedatabill);
            return (float)$bal;
        }//end of the function that returns the wallet balace
        
        public function getAllWalletBalance(){
            $trades = $this->getsum("trades","namount","status","SUCCESS"); //credit
            $benefits = $this->getsum("benefits","amount"); //credit
            $payinfo = $this->getsum("payinfo","amount","status","SUCCESS"); //credit from funding wallet
            $withdraws = $this->getsum("withdraws","amount","status","SUCCESS") + $this->getsum("withdraws","amount","status","PENDING") + $this->getsum("withdraws","amount","status","NEW"); // debit
            $spin = $this->getsum("giveaways","amount"); // debit
            $airtimedatabill = $this->getsum("billsairtimedata","amount","status","SUCCESS"); // debit
            $bal = ($trades + $benefits + $payinfo) - ($withdraws + $spin + $airtimedatabill);
            return $bal;
        }//end of the function that returns the wallet balace
