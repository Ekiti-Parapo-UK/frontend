...
/**
 * ************************************************************************************
 * 	               ezzzySQL PHP CLASS 	v2.0										  *
 * 			(c) 2022	 Ezekiel Aliyu                    							  *
 * ************************************************************************************ 
 */
/**
 * A simple but powerful general purpose MSSQL Class. <br />
 *
 * This is a very <b>simple</b> but <b>powerful</b> general purpose MSSQL Class
 * which can be used to add, update, delete, fetch records from a MSSQL database <br />
 ...
 * You can actually do much more with this very simple and powerful MSSQL Class. <br />
 * EXAMPLES:
 *  <code>
 * $yourclass = new EzzzySql($dcon); // Takes your database connection link as parameter
 * </code>
 *
 * 
 * @version 2.00
 * @package EzzzySql_Class
 * @name EzzzySql
 */
class EzzzySql {
 ...
 */
	 ...
	 * $table = "name_of_table";
	 * // now get your value
	 * $yourclass = new EzzzySql($database_connection);
	...
	function getval($where,$whereval,$table,$toget){
		...
		$mres = sqlsrv_query($this->dcon, $tquery);
		if (!$mres){
			$this->showerror("Could not get record from database");
		}
		$tresult = sqlsrv_fetch_array($mres);
...
/**
 ...
 /**
	...
	function getrow($where,$whereval,$table){
		...
		$mqres = sqlsrv_query($this->dcon, $myqry);
		if (!$mqres){
			$this->showerror("Could not get record from database");
		}
		$mrest = sqlsrv_fetch_array($mqres);
		$this->mresult = $mrest;
		return $this->mresult;
	}
	 ...
	 */
		...
		if ($n == 0) {
			...
			$mres = sqlsrv_query($this->dcon, $tquery);
			if (!$mres){
			  echo "Error Number :". sqlsrv_errors() ." -- Error :". sqlsrv_errors();
		...
		if ($whereb =="" && $wherevalb =="") {
		 ...
		 // check that field or value is a single field or value
			...
			// delete values from the database
				...
				$xqury = sqlsrv_query($this->dcon, $mqry);
				if (!$xqury){
	...
	function getallresult($mtable,$orderby="",$order="",$offset="",$total=""){
		...
		// EXECUTE
		$trest = sqlsrv_query($this->dcon, $mqry);
		if (!$trest){
	...
	function getrecords($table,$where="",$whereval="",$whereb="",$wherevalb="",$wherec="",$wherevalc=""){
      ...
      // check the second options
		...
		if ($where =="" && $whereb =="" && $wherec =="") {
			...
			$reslt = sqlsrv_query($this->dcon, $mqry);
			if (!$reslt) {
      ...
      elseif($where !="" && $whereb !="" && $wherec =="") {
		...
		$threst = sqlsrv_fetch_array($reslt);
  ...
  /**
	...
	function dosearch($words,$table,$sfield,$command="",$orderby="",$order="",$offset="",$total=""){
       ...
       // perform query now
        $outcome = sqlsrv_query($this->dcon, $myqry);
        if (!$outcome) {
  ...
  //////////////////////////////// PRIVATE FUNCTIONS //////////////////////////////////////////////////
   ...
   /**
	...
	function rec_exist($ffld,$fval,$tab,$k=""){
	   ...
	   $trest = sqlsrv_query($this->dcon, $mqry);
	   if (sqlsrv_num_rows($trest) ==1 || sqlsrv_num_rows($trest) > 0) {
	   	// there is a row
   ...
   /**
    ...
    function showpic($where,$whereval,$typ){
      ...
      //let us get the image of this user
        $myresult = sqlsrv_query($this->dcon,"SELECT * FROM members WHERE ".$where."='".$whereval."'") or die("Cannot get the profile picture");
        if(sqlsrv_fetch_array($myresult) == 0){
            $row = sqlsrv_fetch_array($myresult);
            if($row['photo'] == ""){
        ...
        else if(sqlsrv_fetch_array($myresult) > 1){
          $myresult = sqlsrv_query($this->dcon,"SELECT * FROM members WHERE ".$where."='".$whereval."'") or die("Cannot get the profile picture");
          while($myrow = sqlsrv_fetch_array($myresult)){
            $imgg = "../admin/".$myrow['photo'];
          }
        }//end of the else statement that says the profile picture is more than one
        else{
          $myresult = sqlsrv_query($this->dcon,"SELECT * FROM members WHERE ".$where."='".$whereval."'") or die("Cannot get the profile picture");
          $myrow = sqlsrv_fetch_array($myresult);
          $imgg = "../admin/".$myrow['photo'];
