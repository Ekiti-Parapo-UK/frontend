...
function cleana($samy) {
    ...
    $samyb = trim($samy);
    $samyb = preg_replace("/%([A-Za-z]+)%/", "", $samyb);
...
function cleanb($sany) {
    ...
    $sanyb = preg_replace("/%([A-Za-z]+)%/", "", $sany);
...
function myformatDate($mddat) {
    ...
    if (preg_match("/:/", $mddat) && preg_match("/-/", $mddat)) {
...
/**
 * Function to generate 6 Digits PIN Code
 * 
 * @name generateACode
 * 
 * @return string generated code
