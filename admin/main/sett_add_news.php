...
@$soption = trim(sql_safe($_POST["soption"]));
...
if ($_GET['typ'] == "new") {
    ...
    if ($ntitle == "" || $ndesc == "") {
        ...
        display_error("Oops!! It seems you have added this News itme before");
    } else {
        //all is well
        ...
        if (is_uploaded_file($_FILES['pic']['tmp_name'])) {
            ...
            $fields = "title,,descp,,ntype,,adates,,adt,,picture,,soption";
            $vals = "'$ntitle',,'$ndesc',,'NEWS',,'$adate',,'$now',,'$filest',,'$soption'";
            ...
            //we shall now insert the refords
            $fields = "title,,descp,,ntype,,adates,,adt,,soption";
            $vals = "'$ntitle',,'$ndesc',,'NEWS',,'$adate',,'$now',,'$soption'";
            $result = $ezzzy->addlist($fields, $vals, "news");
            if (!$result) {
                $_SESSION["msg"] = '<font color=red><b>Invalid Operation! </b> ' . ' No action taken. Try again<br><br></font>';
            }
